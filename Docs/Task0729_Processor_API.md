...
最终查表校准通过独立接口切换，当前版本默认开启：

```c
Task0729_SetGeneratorCorrection(0U); /* 输出原始测量值 */
Task0729_SetGeneratorCorrection(1U); /* 输出拟合后的值 */
uint8_t enabled = Task0729_GetGeneratorCorrection();
```

切换时会清除上一种状态的帧间平滑历史，避免原始值和拟合值混合。
...
- `mode` 和 `generator_correction_enable` 的维度为1；
...
查表前先乘以 2000 转换为 mVpp。算法使用分段线性反查，放在所有
频率和电压参数计算之后：

```text
基波恢复设置幅值 → 查表得到校准后基波 → 计算一个校准系数
→ 同一系数乘基波、全部谐波、Vpp和Vrms
```

因此谐波比例不会被改变，也不会破坏“基波和谐波合成后整体缩放、
合成峰值不超过基波设置Vpeak”的信号源规则。校准系数只由基波设置
幅值决定，不使用含谐波或高频干扰的总Vpp。

表中存在一个不可逆换挡区：设置90 mV显示90 mV，而设置100 mV显示
89 mV。没有额外档位信号时不能同时精确反演，因此89～90 mV读数映射
为折中值95 mV。实测值大于90 mV后立即切入换挡后的反向支路，例如
90.8 mV恢复为102 mV；不能再从95 mV直接插值到110 mV。其余46个
无歧义标定点均按分段线性曲线精确通过。

完整仿真 `G_Sim_V4.slx` 还在信号源中加入了相反方向的实测模型：

```text
设置幅值 → 基波与谐波合成峰值归一化 → 实测正向曲线
→ 作为ADC实际输入（高频干扰在此后另行叠加）
```

例如设置102 mVpp时，完整仿真的信号源实际输出90.8 mVpp；末端关闭
拟合时看到约90.8 mVpp，开启拟合后由换挡后支路恢复约102 mVpp。
正向曲线对
...
当前版本上电默认为 `CORR ON`；需要查看未经查表的实测值时切换为
`CORR OFF`。
