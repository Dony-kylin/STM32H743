...
恢复系数由采样帧峰值与实测基波幅值之比得到。峰值采用接近全局
峰值的重复局部峰平均，避免ADC噪声或孤立毛刺控制恢复系数。比值
不超过 `1.005` 时按1处理，避免FIR启动瞬态对纯基波产生伪校正。
...
`G_Export_V3_data.c` 的文本文件约438 KB，是因为16384点Hann窗浮点
常量以十进制文本保存；它不等于实际Flash占用。
