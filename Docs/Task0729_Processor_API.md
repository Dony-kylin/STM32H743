...
所有频率、幅值和波形计算之后：
=======
纵轴是信号发生器设置值（mVpp）。算法使用单调分段线性反查，放在
所有频率和电压参数计算之后：
...
89 mV。没有额外档位信号时不能同时精确反演，因此89和90 mV读数均
映射为95 mV，两个原始测试点的最坏误差均为5 mV。其余表内实测点
按分段线性曲线通过。
...
=======
例如设置250 mVpp时，完整仿真的信号源实际输出225 mVpp；末端关闭
拟合时应看到原始值，开启拟合后再由反向表恢复250 mVpp。正向曲线对
>>>>>>> 3f986545169033ebbbb525510b4f1a79879244bb
